...
module.exports = Backbone.View.extend({
    ...
    if(modForm[0].checkValidity()) {
      if (bitCoinReturnAddr != this.userModel.get('refund_address')) {
        saveToAPI(modForm, this.userModel.toJSON(), this.model.get('serverUrl') + "settings", function () {
            ...
            function () {
              messageModal.showModal(window.polyglot.t('errorMessages.saveError'), window.polyglot.t('errorMessages.missingError') + ": " + window.polyglot.t('BitcoinReturnAddress'));
            });
      } else {
  ...
  sendPurchase: function(){
    ...
    this.userModel.fetch({
        ...
        $.ajax({
          ...
          success: function(data){
            if(data.success == true){
              ...
              messageModal.showModal(window.polyglot.t('errorMessages.contractError'), window.polyglot.t('errorMessages.sellerError') +" " + window.polyglot.t('errorMessages.checkPurchaseData'));
      ...
      error: function(){
        messageModal.showModal(window.polyglot.t('errorMessages.getError'));
      }
    });
  },
